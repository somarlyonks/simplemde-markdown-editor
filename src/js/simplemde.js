...
var getBindingName = f => !!Object.values(bindings).find(b => b === f)

var fixShortcut = name => isMac ? name.replace('Ctrl', 'Cmd') : name.replace('Cmd', 'Ctrl')

var isMobile = function () { // FIXME: unecessary complex regex
 ...
 */
function createIcon (options, shortcuts, enableTooltips = true) {
  options = options || {}
  ...
  if (options.title && enableTooltips) {
    ...
    if (isMac) {
      el.title = el.title.replace('Ctrl', '⌘').replace('Alt', '⌥')
    }
  }
...
function getState (cm, pos) {
  ...
  for (let i = 0; i < types.length; i++) {
    ...
    if (data === 'strong') ret.bold = true
      ...
      if (/^\s*\d+\.\s/.test(cm.getLine(pos.line))) ret['ordered-list'] = true
    ...
    else if (data.match(/^header(-[1-6])?$/)) ret[data.replace('header', 'heading')] = true
  }
...
function toggleCodeBlock (editor) {
  ...
  function fencing_line (line) {
    ...
    if (typeof line !== 'object') throw 'fencing_line() takes a \'line\' object (not a line number, or line text).  Got: ' + typeof line + ': ' + line

    return line.styles && line.styles[2] && line.styles[2].indexOf('formatting-code-block') !== -1
  }

  // base goes an extra level deep when mode backdrops are used, e.g. spellchecker on
  const tokenState = token => token.state.base.base || token.state.base
  ...
  function code_type (cm, lineNum, line, firstTok, lastTok) {
    ...
    if (lastTok && tokenState(lastTok).indentedCode) return 'indented'
    // has to be after "indented" check, since first chars of first indented line aren"t marked as such
    if (types.indexOf('comment') === -1) return false
    if (tokenState(firstTok).fencedChars || tokenState(lastTok).fencedChars || fencing_line(line)) return 'fenced'
    return 'single'
  }
  ...
  if (is_code === 'single') {
    ...
    if (cur_start.line !== cur_end.line || cur_start.ch !== cur_end.ch) {
      ...
      for (block_start = cur_start.line; block_start >= 0; block_start--) {
        ...
        if (fencing_line(line)) break
      }
      var fencedTok = cm.getTokenAt({
        line: block_start,
        ch: 1
      })
      var fence_chars = tokenState(fencedTok).fencedChars
    ...
    if (cur_start.line !== cur_end.line || cur_start.ch !== cur_end.ch) {
      ...
      if (cur_end.ch === 0) block_end--
    } else {
      // no selection, search for ends of this indented block
      for (block_start = cur_start.line; block_start >= 0; block_start--) {
        line = cm.getLineHandle(block_start)
        if (line.text.match(/^\s*$/)) continue // empty or all whitespace - keep going
    ...
    var next_line = cm.getLineHandle(block_end + 1),
      ...
      next_line_indented = next_line_last_tok && tokenState(next_line_last_tok).indentedCode
    ...
    (function (i) {
      ...
      if (direction !== undefined) {
        if (currHeadingLevel <= 0) {
          if (direction === 'bigger') text = '###### ' + text
          else text = '# ' + text
        } else if (currHeadingLevel === 6 && direction === 'smaller') text = text.substr(7)
        else if (currHeadingLevel === 1 && direction === 'bigger') text = text.substr(2)
        else {
          if (direction === 'bigger') text = text.substr(1)
          else text = '#' + text
        }
      } else {
        if (size === 1) {
          if (currHeadingLevel <= 0) text = '# ' + text
          else if (currHeadingLevel === size) text = text.substr(currHeadingLevel + 1)
          else text = '# ' + text.substr(currHeadingLevel + 1)
        } else if (size === 2) {
          if (currHeadingLevel <= 0) text = '## ' + text
          else if (currHeadingLevel === size) text = text.substr(currHeadingLevel + 1)
          else text = '## ' + text.substr(currHeadingLevel + 1)
  ...
  if (options.toolbar === undefined) {
    ...
    for (var key in toolbarBuiltInButtons) {
      if (toolbarBuiltInButtons.hasOwnProperty(key)) {
        if (key.indexOf('separator-') !== -1) options.toolbar.push('|')
...
SimpleMDE.prototype.autosave = function () {
  ...
  } else console.info('SimpleMDE: localStorage not available, cannot autosave')
}

SimpleMDE.prototype.clearAutosavedValue = function () {
  if (isLocalStorageAvailable()) {
    if (this.options.autosave === undefined || this.options.autosave.uniqueId === undefined || this.options.autosave.uniqueId === '') {
      console.info('SimpleMDE: You must set a uniqueId to clear the autosave value')
      return
    }
...
SimpleMDE.prototype.createToolbar = function (items) {
  ...
  for (i = 0; i < items.length; i++) {
    ...
    (function (item) {
      const el = item === '|' ? createSep() : createIcon(item, self.options.shortcuts, self.options.toolbarTips)
...
SimpleMDE.prototype.toTextArea = function () {
  ...
  if (wrapper.parentNode) {
    if (this.gui.toolbar) wrapper.parentNode.removeChild(this.gui.toolbar)
